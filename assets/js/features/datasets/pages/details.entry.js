...
const getPreviewNewTabInitialContent = () => {
  const template = document.getElementById('dataset-preview-loading-page-template')
...
const handlePreviewFormSubmit = async event => {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error('The dataset preview request has failed.')
    }
...
const init = () => {
  ...
  const actionsForm = document.querySelector('#dataset-actions-form')
