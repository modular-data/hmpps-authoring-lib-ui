import { type GovukLogoProps } from './govuk-logo.types';

export const GovukLogo = ({
  ariaLabelText,
  rebrand = false,
  useLogotype = true,
  useTudorCrown: defaultUseTudorCrown = true,
  ...restProps
}: GovukLogoProps) => {
  let useTudorCrown = defaultUseTudorCrown;

  useTudorCrown = rebrand ? true : useTudorCrown;

  let svgWidth = useTudorCrown ? 32 : 36;
  svgWidth = useLogotype && !rebrand ? svgWidth + 116 : svgWidth;
  svgWidth = useLogotype && rebrand ? svgWidth + 130 : svgWidth;
  ...
  const logotype = (
    <path
      ...
      transform={useTudorCrown ? undefined : 'translate(8 0)'}
    />
  );
  ...
  return (
    ...
    >
      ...
      {!useTudorCrown ? stEdwardsCrown : tudorCrown}
      {useLogotype && (rebrand ? dotLogotype : logotype)}
    </svg>
  );
};
